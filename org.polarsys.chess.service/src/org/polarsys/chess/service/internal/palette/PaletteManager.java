	...
	private static void setPaletteVisibility(PaletteDrawer paletteDrawer, String viewName,
		...
		if (viewName.equals(CHESSProfileManager.IoT_VIEW)){
			...
			if (diagramName.equals(ViewDiagramAssociations.compositeDiagram) && isComponentActive() ){
					paletteDrawer.setVisible(false);
					if ( paletteId.compareTo("CHESSIoT_CSD_SW_DRAWER")==0||
							paletteId.compareTo("CompositeDiagramFunctionalViewDrawerID")==0 || 	
							paletteId.compareTo("CompositeFunctionalViewContractDrawerID")==0)
						
						//TODO here we should re-engineer the code to allow a more flexible management of additional profile palettes
						paletteDrawer.setVisible(true);
				}
...
//			}
			if (diagramName.equals("InternalBlock")) {
				...
				if (paletteId.compareTo("CHESSIoT_IBD_DRAWER") == 0
						|| paletteId.compareTo("ForeverIBDNodesDrawerID") == 0
						|| paletteId.compareTo("ForeverIBDEdgesDrawerID") == 0)
					paletteDrawer.setVisible(true);
			}
			if (diagramName.equals(ViewDiagramAssociations.blockDiagram)) {
				...
				if (paletteId.compareTo("CHESSIoT_BDD_DRAWER") == 0
						|| paletteId.compareTo("CHESSIoT_BDD_DRAWER_Ports") == 0
						|| paletteId.compareTo("ForeverBDDAssociationsDrawerID") == 0
